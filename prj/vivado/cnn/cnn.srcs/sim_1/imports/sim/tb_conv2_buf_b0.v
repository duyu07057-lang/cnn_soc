...
// Self-checking testbench for the B0 single-bank Conv2 window buffer.
// It verifies all 121 row-major 3x3 windows for two sequential frames, checks
// pulse/count behavior, and exercises bank clear/reuse without parameter files.
module tb_conv2_buf_b0;
    ...
    initial begin
        ...
        if (error_count == 0) begin
            $display(
                "[TB][PASS] B0 single bank produced %0d correct windows across %0d sequential frames.",
                total_windows,
                TEST_FRAMES
            );
            $finish;
            ...
            $fatal(1, "[TB][FAIL] B0 errors=%0d", error_count);
        end
    end
