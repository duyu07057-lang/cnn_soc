...
#
# Modified for:
#   - ALIENTEK Navigator ZYNQ-7020 V1
#   - Vivado 2020.2
#   - AXI DMA + pipelined axis_cnn_mnist at 100 MHz
#   - reproducible creation of a new SoC Block Design
#
...
if { ![string equal -nocase $project_part $expected_part] } {
   catch {common::send_gid_msg -ssname BD::TCL -id 2030 -severity "ERROR" "Current project part is <$project_part>; expected <$expected_part> for the Navigator ZYNQ-7020 V1."}
   return 4
}
...
if { [info exists ::CNN_SOC_DESIGN_NAME] && $::CNN_SOC_DESIGN_NAME ne "" } {
   set design_name $::CNN_SOC_DESIGN_NAME
} else {
...
##################################################################
# CHECK CNN MEMORY INITIALIZATION FILES
##################################################################
...
set missing_mem_files [list]
foreach mem_name $required_mem_files {
   set mem_objects [get_files -quiet -all "*$mem_name"]
   if { [llength $mem_objects] == 0 } {
      lappend missing_mem_files $mem_name
   } else {
      foreach mem_object $mem_objects {
         set_property FILE_TYPE {Memory Initialization Files} $mem_object
         set_property USED_IN_SYNTHESIS true $mem_object
      }
   }
}

if { [llength $missing_mem_files] != 0 } {
   catch {common::send_gid_msg -ssname BD::TCL -id 2024 -severity "ERROR" "Missing CNN memory initialization files: [join $missing_mem_files {, }]. Add all 16 files to Design Sources before sourcing this script."}
   set bCheckIPsPassed 0
} else {
   common::send_gid_msg -ssname BD::TCL -id 2025 -severity "INFO" "CNN memory initialization files: 16/16 PASS."
}
...
proc create_root_design { parentCell } {
  ...
  # Navigator ZYNQ-7020 V1 board settings and PS/PL interfaces.
  # Apply these overrides after the original generated settings so Vivado can
  # recalculate all dependent PS clock and DDR controller values consistently.
  set_property -dict [ list \
...
# Create the HDL wrapper and make it the synthesis top.
set created_bd_file [get_files -quiet "${design_name}.bd"]
if { [llength $created_bd_file] != 1 } {
   catch {common::send_gid_msg -ssname BD::TCL -id 2031 -severity "ERROR" "Cannot locate the newly created ${design_name}.bd file."}
   return 5
}

set wrapper_name "${design_name}_wrapper"
set wrapper_files [get_files -quiet -all "*${wrapper_name}.v"]
if { [llength $wrapper_files] == 0 } {
   set wrapper_path [make_wrapper -files $created_bd_file -top]
   add_files -norecurse $wrapper_path
}
set_property top $wrapper_name [get_filesets sources_1]
update_compile_order -fileset sources_1

# Warn if the copied project still contains the standalone CNN top-level clock
# constraint. The SoC wrapper has no external aclk port; FCLK0 supplies 100 MHz.
set stale_aclk_constraints [list]
set constraint_set [get_filesets -quiet constrs_1]
if { [llength $constraint_set] == 1 } {
   foreach constraint_file [get_files -quiet -of_objects $constraint_set] {
      set constraint_path [get_property NAME $constraint_file]
      if { [file exists $constraint_path] } {
         set constraint_channel [open $constraint_path r]
         set constraint_text [read $constraint_channel]
         close $constraint_channel
         if { [regexp -nocase {create_clock[^\r\n]*get_ports[^\r\n]*aclk} $constraint_text] } {
            lappend stale_aclk_constraints $constraint_path
         }
      }
   }
}

foreach stale_constraint $stale_aclk_constraints {
   puts "CNN-SOC: WARNING: Disable the standalone aclk constraint before synthesis: $stale_constraint"
}

puts ""
puts "CNN-SOC: PASS - Block Design created and validated."
puts "CNN-SOC: Design name       : $design_name"
puts "CNN-SOC: Synthesis top     : $wrapper_name"
puts "CNN-SOC: Device            : $project_part"
puts "CNN-SOC: PL clock          : 100 MHz"
puts "CNN-SOC: UART0             : MIO 14..15, 115200 baud"
puts "CNN-SOC: DMA stream width  : 8 bits"
puts "CNN-SOC: Input FIFO depth  : 8192"
puts "CNN-SOC: Output FIFO depth : 64, TLAST enabled"
puts "CNN-SOC: CNN reset         : peripheral_aresetn"
